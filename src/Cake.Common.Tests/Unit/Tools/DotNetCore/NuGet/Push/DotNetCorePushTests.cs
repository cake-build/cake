    ...
    public sealed class DotNetCorePushTests
    {
        public sealed class ThePushMethod
        {
            ...
            public void Should_Add_Additional_Arguments()
            {
                ...
                // Then
                Assert.Equal(string.Format("nuget push {0} --source {1} --api-key \"{2}\" --symbol-source {3} --symbol-api-key \"{4}\" --no-service-endpoint --timeout {5} --disable-buffering --no-symbols --skip-duplicate --force-english-output", packageName, source, apiKey, symbolSource, symbolApiKey, timeout), result.Args);
            }
