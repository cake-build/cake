...
{
    internal sealed class AssemblyInfoParserFixture_VB
    {
        ...
        public AssemblyInfoParserFixture_VB()
        {
            ...
            // Set fixture values.
            Path = new FilePath("./output.vb");
            CreateAssemblyInfo = true;
        }

        public void WithAssemblyInfoContents(string assemblyInfoContents)
        {
            FileSystem.CreateFile("/Working/output.vb").SetContent(assemblyInfoContents);
        }
        ...
        private void CreateAssemblyInfoOnDisk(FilePath path)
        {
            ...
            if (ExtraWhiteSpaces)
            {
                creator.Create(path, settings, vbAttributeFormat: "<Assembly: {0} >",
                    vbAttributeWithValueFormat: "<Assembly: {0}( {1} )>",
                    vbAttributeWithKeyValueFormat: "<Assembly: {0}( {1}, {2} )>");
            }
            else
            {
